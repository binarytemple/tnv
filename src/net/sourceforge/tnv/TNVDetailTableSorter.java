/**
 * TNVDetailTableSorter is a decorator for TableModels; adding sorting
 * functionality to a supplied TableModel. TNVDetailTableSorter does not store
 * or copy the data in its TableModel; instead it maintains a map from the row
 * indexes of the view to the row indexes of the model. As requests are made of
 * the sorter (like getValueAt(row, col)) they are passed to the underlying
 * model after the row numbers have been translated via the internal mapping
 * array. This way, the TNVDetailTableSorter appears to hold another copy of the
 * table with the rows in a different order.
 * 
 * TNVDetailTableSorter registers itself as a listener to the underlying model,
 * just as the JTable itself would. Events recieved from the model are examined,
 * sometimes manipulated (typically widened), and then passed on to the
 * TNVDetailTableSorter's listeners (typically the JTable). If a change to the
 * model has invalidated the order of TNVDetailTableSorter's rows, a note of
 * this is made and the sorter will resort the rows the next time a value is
 * requested.
 * 
 ...
 * complete UI for TNVDetailTableSorter. The default renderer of the tableHeader
 * is decorated with a renderer that indicates the sorting status of each
 * column. In addition, a mouse listener is installed with the following
 * behavior:
 * 
 ...
 */
public class TNVDetailTableSorter extends AbstractTableModel {
	...
	protected static final Comparator LEXICAL_COMPARATOR = new Comparator() {
	...
	protected TNVDetailTableSorter() {
		this.mouseListener = new MouseHandler();
		this.tableModelListener = new TableModelHandler();
	}


	protected TNVDetailTableSorter(TableModel tableModel) {
		this();
		setTableModel( tableModel );
	}


	protected TNVDetailTableSorter(TableModel tableModel, JTableHeader tableHeader) {
		this();
	...
	protected void setTableHeader( JTableHeader tableHeader ) {
	...
	protected void setColumnComparator( Class type, Comparator comparator ) {
		if ( comparator == null ) {
	...
	private class Row implements Comparable {
		...
		public int compareTo( Object o ) {
			...
			for ( Iterator it = TNVDetailTableSorter.this.sortingColumns.iterator(); it.hasNext(); ) {
				Directive directive = (Directive) it.next();
				int column = directive.column;
				Object o1 = TNVDetailTableSorter.this.tableModel.getValueAt( row1, column );
				Object o2 = TNVDetailTableSorter.this.tableModel.getValueAt( row2, column );
	...
	private class TableModelHandler implements TableModelListener {
		...
		public void tableChanged( TableModelEvent e ) {
			...
			if ( e.getFirstRow() == e.getLastRow() && column != TableModelEvent.ALL_COLUMNS
					&& getSortingStatus( column ) == NOT_SORTED && TNVDetailTableSorter.this.modelToView != null ) {
				int viewIndex = getModelToView()[e.getFirstRow()];
				fireTableChanged( new TableModelEvent( TNVDetailTableSorter.this, viewIndex, viewIndex, column, e
						.getType() ) );
				return;
			}
	...
	private class SortableHeaderRenderer implements TableCellRenderer {
		...
		public Component getTableCellRendererComponent( JTable table, Object value, boolean isSelected,
			...
			Component c = this.tableCellRenderer.getTableCellRendererComponent( table, value, isSelected, hasFocus,
					row, column );
